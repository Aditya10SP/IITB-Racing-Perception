...
def vis_tensor_and_save(image, h, w, tensor_output, image_name, output_uri):
    ...
    points_gopalan = [image_name, 'N/A']
    for pt in np.array(tensor_output):
        cv2.circle(image, (int(pt[0] * w), int(pt[1] * h)), 2, colors[i], -1)
        i += 1
        points_gopalan.append(f'[{int(pt[0] * w)}, {int(pt[1] * h)}]')
    # print(points_gopalan)
    ...
    try:
        ...
        print('File already exists, please view it or try inference on a different image!\n')

    return image, points_gopalan
...
def load_train_csv_dataset(train_csv_uri, validation_percent, keypoint_keys, dataset_path, cache_location=None):
    ...
    if train_labels is None:
        ...
        for i in range(len(labels)):
            ...
            # local_path = dataset_path + str(images[i])
            # image_uris.append(local_path)
